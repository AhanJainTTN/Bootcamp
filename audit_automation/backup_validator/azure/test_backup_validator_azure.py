...
def extract_recovery_point_count(
    ...
    """
    Count valid recovery points for a given VM.
    """
    LOGGER.info(
...
def extract_backup_details(
    ...
    """
    For each protected VM in the vault, extract and return backup details.
    """
    LOGGER.info(
...
def extract_backup_details_for_all_vaults():
    """
    Retrieves backup details for all Recovery Services Vaults across all resource groups in the current subscription.
    """
    LOGGER.info(
...
def process_all_vms():
    ...
    backed_up_vm_df.fillna("N/A", inplace=True)
    custom_remarks = [
        {
...
def main():
    ...
    df_1 = pd.read_excel(
        os.path.expanduser(
            "~/Desktop/backup_validator/azure/test_files/vm_validator_raw_1.xlsx"
        )
    )
    df_2 = pd.read_excel(
        os.path.expanduser(
            "~/Desktop/backup_validator/azure/test_files/vm_validator_raw_2.xlsx"
        )
    )

    raw_df = pd.concat([df_1, df_2], ignore_index=True)
    ...
    raw_df["BackupCount"] = raw_df["BackupCount"].fillna(0)
    add_remarks(raw_df, custom_remarks)

    master_data_file = os.path.expanduser(
        "~/Desktop/backup_validator/azure/test_files/vm_validator_master.xlsx"
    )
    ...
    output_file_path = os.path.expanduser(
        "~/Desktop/backup_validator/azure/test_files/vm_validator_result.xlsx"
    )
