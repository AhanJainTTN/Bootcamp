# Generated by Django 5.1.6 on 2025-03-04 18:40

import menu.validators
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='MenuItem',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=100)),
                ('description', models.TextField(default='Description', max_length=500)),
                ('image', models.ImageField(blank=True, help_text='Please upload an image with equal height and width.', null=True, upload_to='menu/menu_items/', validators=[menu.validators.validate_image])),
                ('rating', models.FloatField(blank=True, default=0.0)),
                ('price', models.DecimalField(decimal_places=2, max_digits=10)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
            ],
        ),
    ]
