...
class FermosaCollectionsScraper:
    ...
    def process_listing(self, listing_url: str) -> Dict[str, str]:
        ...
        # Using a lock ensures correct printing and prevents context switches. This ensures only one thread can increment self.listing_count at a time and it's value is immediately assigned to a local variable which is not shared by threads in this context.
        # Excel output may be random but can be sorted - this ensures no duplicate S.Nos.
        with self.lock:
